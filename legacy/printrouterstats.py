...
def get_client_names(data):
    ...
    if "mesh_sclient_list" in data and isinstance(data["mesh_sclient_list"], dict):
        ...
        client_names.extend(
            [c.get("name") for c in data["mesh_sclient_list"] if "name" in c]
        )
...
print(
    f"{bcolors.OKBLUE}Uptime:{bcolors.ENDC} {bcolors.WARNING}{seconds_to_readable_format(status.wan_ipv4_uptime)}{bcolors.ENDC} | {bcolors.OKBLUE}CPU:{bcolors.ENDC} {bcolors.WARNING}{status.cpu_usage * 100}%{bcolors.ENDC} | {bcolors.OKBLUE}RAM:{bcolors.ENDC} {bcolors.WARNING}{status.mem_usage * 100}%{bcolors.ENDC}"
)
print(
    f"Devices connected: {bcolors.WARNING}{status.clients_total}{bcolors.ENDC} ({bcolors.WARNING}{status.wired_total}{bcolors.ENDC} wired/{bcolors.WARNING}{status.wifi_clients_total}{bcolors.ENDC} wifi)\n"
)
...
for device in mesh_data:
    ...
    print(
        ...
        f"{device_type:20} "
        f"{ip:16} "
        f"{connected_devices:<7} "
        f"{location.replace('_', ' ').capitalize():<20} "
        f"{signal_strength:<6} "
        f"{client_names:<80}"
    )
...
print("")
ethernet_data = client.request(
    "admin/status?form=router&operation=read", "operation=read"
)
...
print(
    f"{'Device Name':30} {'Type':20} {'IP Address':16} {'Transfered':<12} {'Download/Upload':<26} {'Signal':6} {'Link speed':20}"
)
print(
    f"{SEPARATOR * 30} {SEPARATOR * 20} {SEPARATOR * 16} {SEPARATOR * 12} {SEPARATOR * 26} {SEPARATOR * 6} {SEPARATOR * 20}"
)
...
for device in smart_data:
    ...
    print(
        f"{device_name:30} {device_type:20} {ip:16} {data_transfered:<12} {down_up_text:<26} {signal:<6} {link_text:<20}"
    )
